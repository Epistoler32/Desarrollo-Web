...
class Caso1E2ETest extends BaseE2ETest {
    ...
    private static String adminTabHandle;
    private static String menuTabHandle;
    private static String selectedAdicional1;
    private static String selectedAdicional2;
    private static String selectedAdicional3;
    ...
    void loginCorrectoVaDashboard() {
        ...
        // Save admin tab handle for later multi-tab steps
    ...
    void crearNuevoProducto() {
        // Re-authenticate (each @Test gets a fresh browser via @BeforeEach)
        navigateTo("/admin/login");
        adminLoginPage().loginAs(TestData.ADMIN_CORREO, TestData.ADMIN_PASSWORD);
        wait.until(ExpectedConditions.urlContains("/admin/dashboard"));
        adminTabHandle = driver.getWindowHandle();
        ...
        // Wait until we leave /productos/nuevo and land on the list page
    ...
    void asignar2Adicionales() {
        ...
        // Pick the first 2 available adicionales dynamically
        List<String> allNames = pt.getAllAdicionalNames();
        assertThat(allNames).hasSizeGreaterThanOrEqualTo(2);
    ...
    void verificar2AdicionalesEnMenu() {
        // Re-authenticate and set up state
        navigateTo("/admin/login");
        adminLoginPage().loginAs(TestData.ADMIN_CORREO, TestData.ADMIN_PASSWORD);
        wait.until(ExpectedConditions.urlContains("/admin/dashboard"));
        adminTabHandle = driver.getWindowHandle();

        // Assign 2 adicionales (repeat assignment so this test is self-contained)
        navigateTo("/productos");
        ProductosTablePage pt = productosPage();
        pt.waitForLoad();
        pt.openAdicionalesFor(TestData.PRODUCTO_NOMBRE);
        ...
        selectedAdicional1 = allNames.get(0);
        selectedAdicional2 = allNames.get(1);
        selectedAdicional3 = allNames.get(2);
        pt.selectAdicionales(selectedAdicional1, selectedAdicional2);
        pt.saveAdicionales();

        // Open new tab for menu verification
        openNewTab();
        menuTabHandle = driver.getWindowHandle();
        navigateTo("/menu");

        // Find the product card and open its detail
        wait.until(ExpectedConditions.urlContains("/menu"));
        driver.get(driver.getCurrentUrl()); // ensure page is loaded
        // Navigate directly to product detail by searching the page
        wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(
                org.openqa.selenium.By.cssSelector("app-menu-card")));

        // Click the card matching our product name
    ...
    void agregar3erAdicional() {
        ...
        // Restore 2 adicionales first, then add 3rd
        navigateTo("/productos");
        ProductosTablePage pt = productosPage();
        pt.waitForLoad();
        pt.openAdicionalesFor(TestData.PRODUCTO_NOMBRE);
    ...
    void verificar3AdicionalesEnMenu() {
        // Re-authenticate and assign all 3 adicionales
        navigateTo("/admin/login");
        adminLoginPage().loginAs(TestData.ADMIN_CORREO, TestData.ADMIN_PASSWORD);
        wait.until(ExpectedConditions.urlContains("/admin/dashboard"));
